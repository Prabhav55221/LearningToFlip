...
mkdir -p experiments/models/kcoloring/n100/mlp_full_e_norm_nw_rn
mkdir -p experiments/results/ours/run7/kcoloring
mkdir -p logs/slurm

echo "===== Run 7: mlp/full+entropy+norm+noise_walk+reward_norm | kcoloring/n100 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --normalize-features \
    --noise-walk \
    --model-path experiments/models/kcoloring/n100/mlp_full_e_norm_nw_rn/best_mlp_full_e_norm_nw_rn.pt \
    --max-tries 10 \
