...
#SBATCH --output=logs/slurm/interian_kclique_n5_%j.out
#SBATCH --error=logs/slurm/interian_kclique_n5_%j.err
...
mkdir -p experiments/models/kclique/n5/interian
mkdir -p experiments/results/interian/kclique
mkdir -p logs/slurm

echo "===== Interian | kclique/n5 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --model-path experiments/models/kclique/n5/interian/best_interian.pt \
    --max-tries 10 \
    --save-csv experiments/results/interian/kclique/n5_val.csv
END=$(date +%s)
