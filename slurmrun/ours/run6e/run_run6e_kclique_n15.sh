...
mkdir -p experiments/models/kclique/n15/mlp_full_e
mkdir -p experiments/results/ours/run6e/kclique
mkdir -p logs/slurm

echo "===== Run 6e: mlp/full+entropy | kclique/n15 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --feature-set full \
    --model-path experiments/models/kclique/n15/mlp_full_e/best_mlp_full_e.pt \
    --max-tries 10 \
    --save-csv experiments/results/ours/run6e/kclique/n15_val.csv
END=$(date +%s)
