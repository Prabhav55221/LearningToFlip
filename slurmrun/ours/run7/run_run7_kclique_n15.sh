...
mkdir -p experiments/models/kclique/n15/mlp_full_e_norm_fw_sm
mkdir -p experiments/results/ours/run7/kclique
mkdir -p logs/slurm

echo "===== Run 7: mlp/full+entropy+norm+fixed_walk(p=0.1218)+small | kclique/n15 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --n-layers 1 \
    --noise-prob 0.1218 \
    --model-path experiments/models/kclique/n15/mlp_full_e_norm_fw_sm/best_mlp_full_e_norm_fw_sm.pt \
    --max-tries 10 \
