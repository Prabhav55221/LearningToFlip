...
#SBATCH --output=logs/slurm/run7_kclique_n20_%j.out
#SBATCH --error=logs/slurm/run7_kclique_n20_%j.err
...
mkdir -p experiments/models/kclique/n20/mlp_full_e_kl_sm
mkdir -p experiments/results/ours/run7/kclique
mkdir -p logs/slurm

echo "===== Run 7: mlp/full+entropy+kl+small | kclique/n20 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --hidden-dim 32 \
    --n-layers 1 \
    --model-path experiments/models/kclique/n20/mlp_full_e_kl_sm/best_mlp_full_e_kl_sm.pt \
    --max-tries 10 \
