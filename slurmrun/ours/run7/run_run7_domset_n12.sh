...
mkdir -p experiments/models/domset/n12/mlp_full_e_norm_fw_sm
mkdir -p experiments/results/ours/run7/domset
mkdir -p logs/slurm

echo "===== Run 7: mlp/full+entropy+norm+fixed_walk(p=0.0693)+small | domset/n12 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --n-layers 1 \
    --noise-prob 0.0693 \
    --model-path experiments/models/domset/n12/mlp_full_e_norm_fw_sm/best_mlp_full_e_norm_fw_sm.pt \
    --max-tries 10 \
