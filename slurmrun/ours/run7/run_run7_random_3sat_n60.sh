...
mkdir -p experiments/models/random_3sat/n60/mlp_full_e_norm_fw_sm
mkdir -p experiments/results/ours/run7/random_3sat
mkdir -p logs/slurm

echo "===== Run 7: mlp/full+entropy+norm+fixed_walk(p=0.0644)+small | random_3sat/n60 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --n-layers 1 \
    --noise-prob 0.0644 \
    --model-path experiments/models/random_3sat/n60/mlp_full_e_norm_fw_sm/best_mlp_full_e_norm_fw_sm.pt \
    --max-tries 10 \
