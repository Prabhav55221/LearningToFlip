...
#SBATCH --output=logs/slurm/interian_domset_n7_%j.out
#SBATCH --error=logs/slurm/interian_domset_n7_%j.err
...
mkdir -p experiments/models/domset/n7/interian
mkdir -p experiments/results/interian/domset
mkdir -p logs/slurm

echo "===== Interian | domset/n7 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --model-path experiments/models/domset/n7/interian/best_interian.pt \
    --max-tries 10 \
    --save-csv experiments/results/interian/domset/n7_val.csv
END=$(date +%s)
