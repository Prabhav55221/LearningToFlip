...
#SBATCH --output=logs/slurm/interian_domset_n9_%j.out
#SBATCH --error=logs/slurm/interian_domset_n9_%j.err
...
mkdir -p experiments/models/domset/n9/interian
mkdir -p experiments/results/interian/domset
mkdir -p logs/slurm

echo "===== Interian | domset/n9 ====="
echo "Start: $(date)"
TOTAL_START=$(date +%s)
...
python scripts/evaluate.py \
    ...
    --model-path experiments/models/domset/n9/interian/best_interian.pt \
    --max-tries 10 \
    --save-csv experiments/results/interian/domset/n9_val.csv
END=$(date +%s)
